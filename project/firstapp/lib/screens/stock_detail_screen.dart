...
class _StockDetailScreenState extends State<StockDetailScreen> {
  ...
  Future<void> _buy() async {
    ...
    try {
      ...
      final q = _quantity;
      final total = widget.price * q;
      if (existing >= 0) {
        positions[existing]['quantity'] = (positions[existing]['quantity'] as int) + q;
        positions[existing]['totalSpent'] = (positions[existing]['totalSpent'] as num) + total;
      } else {
        positions.add({
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Achat effectué : $q × ${widget.symbol} = ${total.toStringAsFixed(2)} €'),
      ...
      setState(() => _loading = false);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Erreur lors de l\'achat'), backgroundColor: DesignSystem.red500),
      );
    }
  }
}
